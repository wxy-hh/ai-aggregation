...
export interface ResumeInputProps extends React.InputHTMLAttributes<HTMLInputElement> {
  label?: string;
  error?: string;
  helperText?: string;
  /** 最大字符长度限制 */
...
const ResumeInput = React.forwardRef<HTMLInputElement, ResumeInputProps>(
  (
    ...
    const inputId = id || `resume-input-${React.useId()}`;
    const helperId = `${inputId}-helper`;
    const errorId = `${inputId}-error`;

    // 检查字段是否为空或仅空白字符
    const isEmptyOrWhitespace = !inputValue || inputValue.trim().length === 0;
