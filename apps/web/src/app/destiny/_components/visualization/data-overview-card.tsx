...
const ELEMENT_MEANINGS: Record<FiveElementKey, string> = {
  ...
  earth: '代表稳定、承载与诚信，关联脾胃与肌肉，对应季末与中央。土旺则踏实可靠能成事，土弱则缺乏根基感',
  metal: '代表规则、决断与执行，关联肺与皮肤，对应秋季与西方。金旺则条理清晰敢于取舍，金弱则原则感不足',
  water: '代表智慧、变通与内省，关联肾与泌尿系统，对应冬季与北方。水旺则悟性高善于适应，水弱则思维固化',
};

/** 十神大白话映射 */
const TEN_GOD_FRIENDLY_LABELS: Record<string, string> = {
  bijian: '自我表达', jiecai: '自我表达',
  shishen: '创造才能', shangguan: '创造才能',
  zhengcai: '财富积累', piancai: '财富积累',
  zhengguan: '事业成就', qisha: '事业成就',
  zhengyin: '资源支持', pianyin: '资源支持',
};

/** 归并维度在命理中的实际含义 */
const DOMAIN_MEANINGS: Record<string, string> = {
  '自我表达': '比肩与劫财的能量占比。反映你坚持主见、主动争取位置的倾向，数值越高越独立自主、边界感强',
  '创造才能': '食神与伤官的能量占比。反映你输出创意、表达自我的天赋，数值越高越擅长靠才华吸引机会',
  '财富积累': '正财与偏财的能量占比。反映你对资源的感知力和结果导向，数值越高越看重效率与可落地的回报',
  '事业成就': '正官与七杀的能量占比。反映你的规则意识、责任担当与抗压能力，数值越高越能以目标驱动自己',
...
function overcomes(el: FiveElementKey): FiveElementKey[] {
  const map: Record<FiveElementKey, FiveElementKey[]> = {
    metal: ['wood'], wood: ['earth'], water: ['fire'], fire: ['metal'], earth: ['water'],
  };
  return map[el];
}
function overcomeBy(el: FiveElementKey): FiveElementKey[] {
  const map: Record<FiveElementKey, FiveElementKey[]> = {
    metal: ['fire'], wood: ['metal'], water: ['earth'], fire: ['water'], earth: ['wood'],
  };
  return map[el];
}
...
export function DataOverviewCard({
  ...
  const { favorable, unfavorable, favorableReason } = useMemo(() => {
    ...
    const favorableEls = supporting.filter(
      (k) => belowAvg.includes(k) || !aboveAvg.includes(k)
    );
    ...
    const unfavEls =
      highRestraining.length > 0 ? highRestraining : restraining.slice(0, 1);
...
// 日主强弱概述
  ...
  return (
    <GlassCard className={cn('shrink-0 overflow-hidden p-4 sm:p-5', className)}>
      ...
      {/* 喜用神 */}
      <div className="mt-4 rounded-2xl bg-gradient-to-r from-emerald-50/70 to-blue-50/70 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            <span className="text-sm font-extrabold text-emerald-700">喜用神</span>
            {favorable.length > 0 ? (
              favorable.map((k) => (
                <span
                  key={k}
                  className={cn(
                    ...
                    ELEMENT_COLORS[k].bg, ELEMENT_COLORS[k].text,
                  )}
                >
                  {ELEMENT_LABELS[k]}
                </span>
            ...
            {unfavorable.length > 0 && (
              <>
                <span className="text-sm font-extrabold text-amber-700">忌神</span>
                {unfavorable.map((k) => (
                  <span
                    key={k}
                    className={cn(
        ...
        {showFavorableIntro && (
          <div className="mt-2 border-t border-emerald-100/60 pt-2 text-[11px] leading-5 text-slate-500">
            <p>
              ...
              生活中可多接触喜用神对应的颜色、方位、行业，有助于增强运势。<br />
              {favorableReason}
            </p>
            <p className="mt-1.5">
                        ...
                        >
                          ...
                          <div className="space-y-1.5 text-xs text-slate-600">
                            ...
                            <div className="flex justify-between border-t border-slate-100 pt-1.5 mt-1.5">
                              <span>月令（季节加成）</span>
                              <span className={cn('font-semibold', hasSeasonalBonus ? 'text-amber-600' : 'text-slate-400')}>
                  ...
                  return (
                    <div key={item.key}>
                      <div className="flex items-center justify-between gap-3">
                        <Popover>
                          <PopoverTrigger asChild>
                            <button type="button" className="text-sm font-semibold text-slate-700 cursor-help text-left">
                              {item.label}
                            </button>
                          </PopoverTrigger>
                          <PopoverContent side="top" className="w-52 rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
                            <div className="text-xs font-bold text-slate-700">{item.label}</div>
                            <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
                              属于"{domainLabel}"维度的能量。{item.label}数值越高，说明该十神在命局中的作用越突出，对该维度的外在表现与内在倾向均有直接影响。
                            </p>
                          </PopoverContent>
                        </Popover>
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <Popover>
                          <PopoverTrigger asChild>
                            <button type="button" className="text-sm font-semibold text-slate-700 cursor-help text-left">
                              {item.friendlyLabel}
                              <span className="ml-1 text-slate-400 font-normal">（{item.labels.join('、')}）</span>
                            </button>
                          </PopoverTrigger>
                          <PopoverContent side="top" className="w-56 rounded-xl border border-slate-200 bg-white p-3 shadow-lg">
                            <div className="text-xs font-bold text-slate-700">{item.friendlyLabel}</div>
                            <p className="mt-1 text-[11px] leading-relaxed text-slate-500">
