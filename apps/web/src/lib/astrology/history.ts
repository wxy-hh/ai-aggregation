...
import { buildMockInterpretation } from './mock-interpretation';
 ...
 * - 摘要：黄金主轴一句话（不含出生时刻/经纬度等高敏字段）
 * - coreTone 徽章：太阳×月亮星座的极简组合
 */
function buildTitleAndPreview(formData: AstrologyFormData, facts: AstrologyChartFacts) {
  const name = formData.name.trim() || '星盘主人';
  const title = `星座寰宇 · ${name}的本命星盘`;

  const interpretation = buildMockInterpretation(facts);
  ...
  // 主轴是金句而非隐私数据；拿不到主轴时退化为日月星座组合
  const preview = (interpretation.headline?.text || fallbackPreview).slice(0, 150);
