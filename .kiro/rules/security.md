...
- [ ] 所有用户输入均已验证
- [ ] 预防 SQL 注入（使用参数化查询/Parameterized Queries）
- [ ] 预防跨站脚本攻击（XSS）（对 HTML 进行净化处理/Sanitized）
- [ ] 已启用跨站请求伪造（CSRF）保护
- [ ] 身份验证（Authentication）与授权（Authorization）已验证
...
3. 在继续之前修复严重（CRITICAL）级别的问题
4. 轮换（Rotate）任何已暴露的凭据
5. 审查整个代码库是否存在类似问题
