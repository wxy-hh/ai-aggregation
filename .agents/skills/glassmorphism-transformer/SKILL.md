...
### 6.1 AI 行为反例（本技能自身不得执行的操作）

| # | 禁止行为 | 原因 |
|---|---|---|
| 1 | **修改 DESIGN.md 自身** | DESIGN.md 是全局设计规范源头，非本技能资产 |
| 2 | **覆盖用户完整的 `tailwind.config.js`** | 仅追加 `theme.extend` 段，保留用户原有配置 |
| 3 | **在 > 1 屏高度的滚动容器上直接加 `backdrop-blur`** | 导致移动端严重掉帧，必须降级处理 |
| 4 | **在复杂表单/大段文本区应用强透明玻璃** | 破坏可读性，违反内容独立性原则 |
| 5 | **跳过步骤 2 基础设施检查直接开始改造** | 缺失 `backdropBlur`/`boxShadow` 配置会导致类名无效 |
| 6 | **交付未通过 🛑 STOP 自检的代码** | 禁忌表任一不通过即为不合格交付 |
