...
> 状态：已确认的产品与交互方案
> 适用范围：AI 命理大师的八字结果页、合盘信息引导页、合盘报告页；移动端与桌面端均为完整体验
> 设计图：
> - [入口位置效果图，移动端](./2026-07-25-bazi-compatibility-entry-mobile.png)
> - [入口位置效果图，桌面端](./2026-07-25-bazi-compatibility-entry-desktop.png)
> - [补充 TA 信息引导图，移动端](./2026-07-25-bazi-compatibility-partner-info-mobile.png)
> - [补充 TA 信息引导图，桌面端](./2026-07-25-bazi-compatibility-partner-info-desktop.png)
> - [独立合盘报告效果图，移动端](./2026-07-25-bazi-compatibility-result-mobile-v2.png)
> - [独立合盘报告效果图，桌面端](./2026-07-25-bazi-compatibility-result-desktop.png)
...
桌面端沿用与移动端完全相同的纵向位置，即四柱命盘之后、排盘依据之前；区别在于它不需要占满整行手机宽度，而是作为主报告列中的横向关系入口。右侧的人生五维或报告目录保持个人报告内容，不能提前显示合盘结论。

- 宽屏下，入口卡与四柱命盘使用同一主报告列，建议主列宽度为 8/12；右侧 4/12 保持个人分析侧栏。
- 入口从左到右依次为“我 × TA”关系标识、价值文案、适用范围、主按钮。主按钮始终可见，不依赖悬停展开。
- 入口是可选的下一步，视觉强调低于四柱命盘，高于排盘依据；使用浅色关系连接线和完整描边，不用强烈大面积渐变或浮层阴影抢走个人报告焦点。
- 当鼠标悬停入口时，只增强边框和连接线亮度；键盘聚焦和点击效果与悬停等价。
...
桌面端不是把移动端的单列字段拉宽，而是把“填写”与“为什么填写”同时放在用户视线内。

| 区域 | 桌面端布局 | 用户价值 |
| --- | --- | --- |
| 顶部关系条 | `我 × TA`、四种关系 Tab、当前关注在一行呈现 | 不离开表单也能确认解读场景。 |
| 主表单区 | 8/12 宽，两列字段：称呼/性别、历法/日期、时间/地点；可选问题与授权确认置于表单末尾 | 减少纵向滚动，字段间的关联更清楚。 |
| 说明侧栏 | 4/12 宽，展示“你的命盘已就绪”、本次会解读的三项内容、出生时间缺失说明和隐私承诺 | 用户不必中断填写去找帮助信息。 |
| 底部操作条 | 左侧返回个人报告与文化提示，中间或右侧放唯一蓝色主按钮 | 主操作明确，同时不让免责声明遮挡提交。 |

- 表单区使用实体浅色阅读面，不在每个字段外再包一层玻璃卡；输入控件保持项目既有的圆角、边框、聚焦环和禁用状态。
- 桌面端允许鼠标键盘快速填写，日期、时间和地点保留可直接键入与下拉选择两种方式。
- 说明侧栏在 `1024px` 以下移到主表单下方，不能压缩字段宽度至不可读。
...
4. 切换未缓存关系视角会重新调用模型生成该视角，并与首开一样预扣/结算额度；加载态不展示「不额外扣费」。额度不足时走全局额度弹框，并回退到已缓存视角。
...
- 关系底色使用一块主视觉面板承载，常规阅读与数据区域使用高对比度的实体浅色表面；玻璃仅用于工作区外壳、资料摘要和主视觉，不能让每个内容单元都漂浮起来。
- 桌面端让宝蓝色只承担选中、主操作和图谱数据，TA 的珊瑚色只承担身份区分；移动端沿用同一语义，不能因屏幕变小而增加高饱和装饰。
...
桌面端与移动端共享同一份合盘事实、关系类型、结论优先级和行动建议，不能因屏幕不同而改变信息架构或隐藏核心能力。两端的差异只来自使用场景：桌面端适合并行阅读与深度对照，移动端适合单手浏览与立即行动。

| 共同内容 | 两端都必须可见 | 呈现差异 |
| --- | --- | --- |
| 导航与关系上下文 | 返回、我的八字、我 × TA、关系类型、资料完整度 | 桌面端是页头命令条与资料横条；移动端是安全区页头与紧凑资料行。 |
| 关系底色 | 合拍指数、结论、为什么这么说 | 桌面端与年度节奏并列；移动端独占首屏主视觉。 |
| 双向需求与六维图谱 | 我与 TA 的核心需求、六项默契度、最高/最低项解释 | 桌面端并行展示；移动端依阅读顺序纵向呈现。 |
| 洞察与行动 | 彼此吸引、相处提醒、年度节奏、今天的行动 | 桌面端可一屏扫描；移动端以可持续滚动与底部主操作呈现。 |
| 关系 Tab | 恋爱、婚姻、朋友、合作 | 桌面端在资料条下方横向排列；移动端保持单行等宽、可触达。 |

### 14.2 断点与桌面端布局

| 视口 | 工作区结构 | 内容排布 | 导航与操作 |
| --- | --- | --- | --- |
| `≥ 1280px` 宽屏桌面 | 64px 全局图标栏 + 228px 命理二级导航常驻；主内容最大宽度 1360px | 12 列网格：关系底色占 8 列，年度节奏占 4 列；六维图谱占 8 列，双向需求占 4 列；下方洞察横向阅读 | 顶部固定命令条展示“查看我的八字报告、重新填写资料、生成缘分卡”，所有操作文字可见。 |
| `1024–1279px` 笔记本 | 全局图标栏保留；二级导航可收起为抽屉或窄栏 | 主内容改为 7/5 列；年度节奏保留右侧，洞察区允许分两行 | 顶部保留文字主操作；次要操作收进更多菜单，但“我的八字”始终可见。 |
| `768–1023px` 平板 | 左侧导航改为可显式打开的抽屉 | 关系底色与年度节奏上下排列；六维图谱仍保持 3 × 2；双向需求可左右并列 | 使用顶部工具栏，生成缘分卡保持可见，不依赖悬停。 |

桌面端具体规格：

- 主内容区使用稳定的 12 列网格，页内留白为 32px，模块间距为 16px 或 24px；宽屏不将阅读区无限拉长。
- 顶部资料条是单行上下文，不做第二张大卡；它展示“我 × TA、恋爱、双方资料完整、当前关注”，帮助用户随时确认当前正在阅读哪一段关系。
- 关系底色与年度节奏并列，使用户先看到“关系是什么”，再看到“下一步什么时候做什么”；右栏不承载个人八字信息。
- 六维图谱使用 3 × 2 的紧凑数据单元，数值、名称和进度同时显示；双向需求采用左右分区而不是两个重复的浮层卡片。
- 鼠标悬停只增强已可见的信息，例如显示“为什么这么说”的命盘依据摘要；不得把核心操作或核心解释藏在悬停中。键盘焦点、Enter 和 Space 必须可操作同一功能。

### 14.3 移动端布局

- 页面为单列稳定滚动，不使用内部滚动容器；顶部资料行和关系 Tab 在滚动时保持可见或以紧凑形式吸顶。
- 关系底色独占首屏，先给结论、双向需求和六维图谱；“彼此吸引、相处提醒、年度节奏”按操作价值继续向下排列。
- 六维图谱维持 2 × 3 网格，单元内只保留图标、名称、数值和进度，不放长段解释；最低项的行动提示统一置于图谱底部。
- 底部固定一个主按钮 `生成缘分卡`，`重新填写资料` 降级为文字次操作，避免出现两个同权重的大按钮。
- 表单不采用嵌套滚动；输入焦点时底部操作区随键盘上移且不遮挡当前字段；安全区内边距完整覆盖 iPhone 手势区域。
- 关系类型四个 Tab 在窄屏保持单行等宽；图标不能替代中文标签。长称呼和长地点允许两行截断，不挤压主操作按钮。

### 14.4 无障碍与状态

- 进度条、图谱与合拍指数不能只靠颜色表达；必须有文字标签、数值和具体解释。
- 所有图标按钮提供可读标签，例如「返回」「查看我的八字报告」「为什么这么说」。
- 低分或异常提醒不只用红色，需使用中性图标和可执行文案。
- 支持系统的“减少动态效果”和字号放大；关键内容在 200% 放大时保持顺序与可操作性。
- 桌面端与移动端的所有正文文字均满足 WCAG 2.1 AA 对比度；信息状态用图标、文案和颜色三重表达。
...
- [ ] 桌面端入口位于主报告 8/12 列内，右侧个人分析栏保持个人内容，不提前泄露合盘结论。
...
- [ ] 桌面端在 `≥ 1280px` 时使用表单 8/12 + 说明侧栏 4/12 布局；在 `1024px` 以下侧栏按规则下移，不造成字段裁切。
...
- [ ] 1280px 宽度下常驻命理导航、12 列报告网格、年度节奏右栏和三项顶部操作均完整可用。
- [ ] 1024px 与 768px 的过渡宽度下，侧栏、年度节奏、六维图谱和操作栏按规则重排，不能重叠、裁切或依赖悬停。
- [ ] 桌面端“查看我的八字报告”、移动端“我的八字”指向同一关联个人报告；各端返回规则一致。
