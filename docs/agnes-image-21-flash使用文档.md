...
| extra_body.image           | string[] | 图生图必填 | 输入图片数组，支持公网 URL 或 Data URI Base64 |
...
- 图生图请求中，请将输入图片放在 `extra_body.image` 数组中。
...
const response = await fetch('https://apihub.agnes-ai.com/v1/images/generations', {
  ...
  body: JSON.stringify({
    model: 'agnes-image-2.1-flash',
    prompt: 'Make the scene winter with snow while preserving the original composition',
    size: '1024x768',
    extra_body: {
...
```javascript
const imageBase64 = 'iVBORw0KGgo...'; // 图片的 Base64 编码
...
|--------|------|
| 200 | 成功 |
| 400 | 请求参数错误（如 `response_format` 放在顶层） |
| 401 | API Key 无效 |
| 429 | 请求频率超限 |
| 500 | 服务端错误 |
...
建议客户端超时时间设置为：**60s ~ 360s**

---

## 5. 图生图请求缺少 `image`

图生图请求中，`extra_body.image` 数组为必填。
...
- 图生图请求中，请将输入图片 URL 或 Data URI Base64 放在 `extra_body.image` 数组中。
